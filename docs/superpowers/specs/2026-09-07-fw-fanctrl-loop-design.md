...
- **Demand-limited freeze (per axis).** The integrator must not wind up while nothing is
  consuming the budget: at idle or in a game lull the temperature error stays positive, and an
  unchecked integrator saturates at `cpu_max_w + gpu_max_w` so the next load onset runs uncapped
  through the whole dead time. The old design's conservative start and model contour covered
  this; they are deleted, and this replaces them. The rule is a **freeze, not a pull-down**:
  each tick the controller compares the smoothed **measured** draw per axis (`Sample.cpu_pkg_w`
  from RAPL, `Sample.gpu_w` from NVML, both already sampled, 5-sample tail-mean like the fan
  window) against that axis's own commanded cap. An axis is *demand-limited* when its draw sits
  more than `DEMAND_MARGIN_W = 3` below its cap. If **every** axis is demand-limited the cap is
  not the binding constraint, so `Freeze::DemandLimited` holds `u` exactly where it is. If **any**
  axis is at its cap the loop still has authority through that axis and integrates normally.
  Two failure modes this shape exists to avoid, both found by roast iteration 2 in an earlier
  draft that back-calculated `u` toward the measured draw on every tick:
  - **Never pull `u` down toward the draw.** An ungated back-calculation is not anti-windup; it
    is a tracker. Its equilibrium is `u = draw + Kc·e`, so every lull drags the cap onto the
    lull's consumption and the next onset is power-starved for minutes while the integrator
    recharges at a few watts per minute. Classical back-calculation is only ever meant to act
    while the actuator saturates, and against the **bounds** — which is what the clamping rule
    above already does, and the only back-calculation this design performs.
  - **Judge each axis separately.** Comparing the combined sum makes any structurally undrawn
    component (the GPU floor share while the dGPU is unpowered, on a CPU-only workload) a
    permanent gap, which a sum-based rule would try to close by lowering `u` until the CPU is
    pinned at its floor for the session. Per-axis, that idle GPU share simply marks the GPU
    demand-limited while the CPU axis, at its cap, keeps the loop live.
...
- `sensors/ec.rs` (2.2) at the 1 Hz sampler tick. **The NVMe reader does not run there**: a
  ...
  drive stall the control loop — for a value that now only feeds a flag (§2.8). It is polled on
  the existing `FanctrlPoller` thread at 30 s alongside `print all` and cached with a stamp; a
  missing or stale reading yields `None`, which already deactivates the guard. `Sample` gains
