//! Per-device thermal guards. GPU heat ratchets the GPU DeviceLoop maximum
//! toward its configured floor; recovery reopens that ceiling only below the
//! hysteresis threshold. NVMe heat is reported without changing control.
...
/// Per-tick guard flags. NVMe remains reporting-only; GPU actuation is
/// represented by the ratchet maximum returned separately.
...
mod tests {
    ...
    fn guard_state_carries_only_the_two_flags() {
        ...
        // GuardState (an effective target or cap delta — the absent
