...
use crate::control::device_loop::ThermalMode;
...
impl TStarSource {
    pub fn new(seed: EntrySeed) -> Self {
        let qualified_seed = match seed { EntrySeed::Qualified(value) => Some(value), _ => None };
        Self {
    ...
    pub fn tick(&mut self, input: &TStarInput) -> TStarOutput {
        ...
        let reconciliation_recovered = self.last_reconciled == Some(false) && input.replica_reconciled;
        ...
        if !input.fan_valid || !input.ec_valid || input.watchdog_release || input.auto_exit {
            ...
            if self.target.is_none() || reengaged {
                self.target = Some(if reengaged { self.reengagement_target(input, bounds) } else { self.entry_target(input, bounds) });
                self.dirty = true;
                reseed = true;
                resync = true;
            }
            ...
            let debounced = self.advance_argmax_debounce(argmax, input.fresh_view, input.argmax_lead_c);
            let before_key = self.cached_curve_key.clone();
            let curve_target = self.curve_target(input, bounds);
            let view_usable = input.fresh_view && input.replica_reconciled && curve_target.is_some();
            ...
            self.advance_gate(argmax, self.state == TStarState::Held && curve_entry_eligible, dt);
            ...
            if let Some(label) = self.new_stuck_label() {
                ...
                self.pending_stuck = None;
                self.state = TStarState::Held;
                let usable: Vec<&SensorReading> = input.sensors.iter().filter(|r| !self.quarantines.contains_key(&r.label) && r.value_c.is_finite() && r.value_c > 0.0).collect();
                bounds = feasibility(usable.iter().map(|r| (r.class, r.value_c)), input.cpu_hot_c, input.gpu_hot_c);
                flags.retain(|flag| !matches!(flag, TStarFlag::EcUncontrollableUnavailable));
                if !bounds.uncontrollable_available { flags.push(TStarFlag::EcUncontrollableUnavailable); }
            ...
            if reengaged || quarantine_recovered || reconciliation_recovered || quarantined || argmax_quarantined || class == Some(SensorClass::Unknown) {
                self.state = TStarState::Held;
            } else {
                match self.state {
                    ...
                    TStarState::Held if held_to_curve => {
                        self.enter_curve(curve_target.expect("eligible curve"), bounds, before_key, &mut restore_thermal);
                    }
                    TStarState::Curve if controllable_view => {
                        ...
                        self.enter_curve(curve_target.expect("usable curve"), bounds, before_key, &mut restore_thermal);
                    }
    ...
    fn reengagement_target(&self, input: &TStarInput, bounds: Feasibility) -> f64 {
        self.qualified_seed.unwrap_or_else(|| input.cpu_group_c.into_iter().chain(input.gpu_group_c).reduce(f64::max).unwrap_or(bounds.ceiling_c)).clamp(bounds.floor_c, bounds.ceiling_c)
    }
    ...
    fn advance_argmax_debounce(&mut self, argmax: Option<&SensorReading>, fresh: bool, lead_c: f64) -> bool {
        let Some(r) = argmax.filter(|r| fresh && matches!(r.class, SensorClass::Controllable | SensorClass::KnownUncontrollable)) else {
            self.argmax_label = None;
            self.argmax_streak = 0;
            return false;
        };
        ...
        lead_c.is_finite() && lead_c > DECISIVE_ARGMAX_LEAD_C || self.argmax_streak >= ARGMAX_DEBOUNCE_SAMPLES
    }
    ...
    fn enter_curve(
        ...
        *restore_thermal = self.target.is_some_and(|old| next > old)
            && self.cached_curve_key != before_key;
        self.set_target(next, bounds);
    }
    fn observe_stuck(&mut self, argmax: Option<&SensorReading>, input: &TStarInput, debounced: bool) {
...
mod tests {
    ...
    fn every_curve_gate_condition_resets_the_continuous_entry_window() {
        ...
        assert_eq!(source.tick(&i).state, TStarState::Held);
        for _ in 0..14 { assert_eq!(source.tick(&i).state, TStarState::Held); }
        assert_eq!(source.tick(&i).state, TStarState::Curve);

        let mut source = TStarSource::new(EntrySeed::Fallback(70.0));
        let mut invalid = input(); invalid.argmax_lead_c = 2.0; invalid.curve_points = None;
        for _ in 0..30 { source.tick(&invalid); }
    ...
    fn cleared_quarantine_restarts_hysteresis_and_stays_held_on_the_30th_sample() {
        ...
        i.sensors = vec![ambient(80.0), cpu(70.0)];
        let mut source = TStarSource::new(EntrySeed::Fallback(70.0));
        for _ in 0..318 { source.tick(&i); }
        i.sensors[0].value_c = 81.0;
        i.argmax_lead_c = 0.5;
        for _ in 0..29 { source.tick(&i); }
        let out = source.tick(&i);
        assert!(!source.quarantined("ambient_f75303@4d"));
        assert_eq!(out.state, TStarState::Held);
        assert!(out.reseed && out.resync);
        for _ in 0..2 { assert_eq!(source.tick(&i).state, TStarState::Held); }
    ...
    fn downward_curve_and_held_changes_report_deltas_without_thermal_restore() {
        let mut i = input(); i.argmax_lead_c = 2.0; i.snapped_duty = Some(37);
        let mut source = TStarSource::new(EntrySeed::Fallback(70.0));
    ...
    fn qualified_seed_wins_and_persistence_rekeys_a_sanitised_target() {
        let mut i = input(); i.fresh_view = false; i.requested_fan_target_rpm = 99_999;
        let mut source = TStarSource::new(EntrySeed::Qualified(75.0));
        let first = source.tick(&i);
        assert_eq!(first.t_star, Some(75.0));
        assert_eq!(first.persistence.expect("initial valid save").seed.fan_target_rpm, 7000);
        i.strategy = Some("cool16".into());
        source.set_held_target(74.0);
        for _ in 0..59 { source.tick(&i); }
    ...
    fn persistence_rejects_released_missing_groups_empty_feasibility_and_quarantine() {
        let mut released = input(); released.fan_valid = false;
        assert!(TStarSource::new(EntrySeed::Fallback(70.0)).tick(&released).persistence.is_none());

        let mut no_groups = input(); no_groups.fresh_view = false; no_groups.cpu_group_c = None; no_groups.gpu_group_c = None;
        assert!(TStarSource::new(EntrySeed::Fallback(70.0)).tick(&no_groups).persistence.is_none());

        let mut no_floor = input(); no_floor.fresh_view = false; no_floor.sensors = vec![cpu(70.0)];
        assert!(TStarSource::new(EntrySeed::Fallback(70.0)).tick(&no_floor).persistence.is_none());

        let mut stuck = input(); stuck.argmax_label = Some("ambient_f75303@4d".into()); stuck.argmax_lead_c = 2.0; stuck.sensors = vec![ambient(80.0), cpu(70.0)];
        let mut source = TStarSource::new(EntrySeed::Fallback(70.0));
        let mut out = source.tick(&stuck);
        for _ in 0..317 { out = source.tick(&stuck); }
        assert!(source.quarantined("ambient_f75303@4d"));
    ...
    fn established_curve_uses_argmax_debounce_before_bypass_and_decisive_leads_bypass_now() {
        let mut i = input(); i.argmax_lead_c = 2.0;
        let mut source = TStarSource::new(EntrySeed::Fallback(70.0));
        to_curve(&mut source, &i);
        i.argmax_label = Some("ambient_f75303@4d".into()); i.argmax_lead_c = 0.5;
        assert_eq!(source.tick(&i).state, TStarState::Curve);
        assert_eq!(source.tick(&i).state, TStarState::Curve);
        assert_eq!(source.tick(&i).state, TStarState::Uncontrollable);

        let mut source = TStarSource::new(EntrySeed::Fallback(70.0));
        let mut decisive = input(); decisive.argmax_lead_c = 2.0;
    ...
    fn uncontrollable_exit_debounces_to_held_then_requires_a_new_curve_window() {
        let mut i = input(); i.argmax_label = Some("ambient_f75303@4d".into()); i.argmax_lead_c = 2.0;
        let mut source = TStarSource::new(EntrySeed::Fallback(70.0));
        ...
        i.argmax_label = Some("cpu@4c".into()); i.argmax_lead_c = 0.5;
        for _ in 0..2 {
            let out = source.tick(&i);
            assert_eq!(out.state, TStarState::Uncontrollable);
    ...
    fn released_reengagement_reuses_qualified_seed_instead_of_current_groups() {
        let mut i = input(); i.fresh_view = false; i.cpu_group_c = Some(65.0); i.gpu_group_c = Some(60.0);
        let mut source = TStarSource::new(EntrySeed::Qualified(75.0));
        assert_eq!(source.tick(&i).t_star, Some(75.0));
        i.ec_valid = false; source.tick(&i);
        i.ec_valid = true;
        let out = source.tick(&i);
        assert_eq!(out.state, TStarState::Held);
    ...
    fn quarantine_tick_recomputes_the_empty_feasibility_flag() {
        ...
        for _ in 0..317 { out = source.tick(&i); }
        assert!(source.quarantined("ambient_f75303@4d"));
    ...
    fn decisive_uncontrollable_argmax_bypasses_immediately_from_held() {
        ...
        assert_eq!(out.state, TStarState::Uncontrollable);
        assert!(out.mode_transfer);
        assert!(out.flags.contains(&TStarFlag::ArgmaxUncontrollable("ambient_f75303@4d".into())));
    }
}
